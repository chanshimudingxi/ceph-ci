...
void *Accepter::entry()
{
  ...
  while (!done) {
    ...
    if (r < 0) {
      ...
      if (hb) {
        if (!unhealthy) {
	  ldout(msgr->cct,0) << "pool got error"
    ...
    if (pfd[0].revents & (POLLERR | POLLNVAL | POLLHUP)) {
      ...
      if (hb) {
        if (!unhealthy) {
	  ldout(msgr->cct,0) << "pool got errors in revents"
	      		  << " reset heartbeat timeout"<< dendl;
          msgr->cct->get_heartbeat_map()->reset_timeout(hb,0,0);
          unhealthy = true;
        } else {
	...
	ldout(msgr->cct,1) << __func__ << " set_close_on_exec() failed "
      ...
      if (++errors > msgr->cct->_conf->ms_accept_error_nums) {
        if (hb) {
	  if (!unhealthy) {
	    ldout(msgr->cct,0) << "accetper has encoutered enough errors,  reset heartbeat timeout." << dendl;
            msgr->cct->get_heartbeat_map()->reset_timeout(hb,0,0);
            unhealthy = true; 
          } else {
            sleep(msgr->cct->_conf->ms_simple_accepter_sleep_time);
